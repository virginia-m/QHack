...
def simple_circuits_50(angle):
    ...
    def circuit(rotation_angle):
        """ Defines a Pennylane circuit on two qubits with a parameterized Y rotation
            on qubit 0 on a Bell state between 0 and 1, followed by a measurement of
            qubit 0 and 1.
        """
        qml.Hadamard(wires=0)
        qml.CNOT(wires=[0, 1])
        qml.RY(rotation_angle, wires=0)
